<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="农药分类" prop="nongyaofenlei">
          <el-input v-model="ruleForm.nongyaofenlei" 
              placeholder="农药分类" clearable  :readonly="ro.nongyaofenlei"></el-input>
        </el-form-item>
        <div v-else>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `nongyaofenlei/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.nongyaofenleiCrossAddOrUpdateFlag = false;
                  this.parent.search();
