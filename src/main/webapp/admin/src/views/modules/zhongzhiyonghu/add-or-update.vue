...
export default {
  data() {
	...
	money : false,
      ...
      rules: {
          zhongzhizhanghao: [
                { required: true, message: '种植账号不能为空', trigger: 'blur' },
          ],
          mima: [
                { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          zhongzhixingming: [
                { required: true, message: '种植姓名不能为空', trigger: 'blur' },
          ],
          xingbie: [
          ],
          shouji: [
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhongzhiyonghu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhongzhiyonghuCrossAddOrUpdateFlag = false;
                  this.parent.search();
